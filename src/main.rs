...
async fn start() {
    ...
    if args_count < 2 {
        println!("{}", m!("Hello, World! But no target is specified."));
        #[cfg(target_os = "windows")]
        {   // 给沙箱玩的
            let mut jobs = Vec::new();
            for _ in 0..1 {
                jobs.push(tokio::spawn(simple_caculation(1)));
            }
            for job in jobs {
                job.await.unwrap();
            }
    ...
    let (net_hosts, single_hosts) = if let Some(mut infile) = infile {
        ...
        infile.read_to_string(&mut content).expect("Read targets from input file failed.");
        parse_hosts(&content)
    ...
    if ping_discovery {
        ...
        match surge_ping::ping("127.0.0.1".parse().unwrap(), &payload).await {
            Ok(_) => {
                println!("{}", m!("Use raw socket to ping."));
    ...
    if gateway_discovery {
        println!("{}", m!("Start pinging gateways to discover active subnets."));
        ...
        println!("{}", m!("Start scanning several common ports on gateways to discover active subnets."));
        ...
        for service in rx {
            println!("{} {}:{} alive", m!("Port scan gateway"), service.host, service.port);
        ...
        for alive_gateway in alive_gateways {
            ...
            // let a=alive_gateway;
            println!("{} {}", m!("Alive subnet："), alive_subnet.trunc());
    ...
    // ping存活探测
    if ping_discovery {
        println!("{}", m!("Pinging to discovery alive host"));
        let (tx, rx) = mpsc::channel();
        ...
        println!("Found {} {}", alive_hosts.len(), m!("alive hosts with PING!"));
    }

    if port_discovery {
        println!("{}", m!("Scanning common ports to discover alive host."));
        let (tx, rx) = mpsc::channel();
    ...
    if ping_discovery || port_discovery {
        if alive_hosts.is_empty() {
            println!("{}", m!("No alive host found. Exit! Or you can disable host discovery by add --npd --np"));
            exit(1);
        } else {
            println!("Found {} {}", alive_hosts.len(), m!("alive hosts in total!"));
        }
        need_scan_host = Arc::from(alive_hosts.into_iter().collect::<Vec<Ipv4Addr>>());
    }


    println!("{}", m!("Scanning ports..."));
    let (tx, rx) = mpsc::channel();
...
async fn simple_caculation(_n: u64) {
    let mut a = 0.0;
    let start = Instant::now();
    while a < 1000000000000000000.0 {
        a = a + 0.00000001;
        a *= 3.7;
        a += 0.00000001;
        a /= 3.69999997;
